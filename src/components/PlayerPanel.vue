      ...
      >
        <div class="player-top">
          ...
          <div class="player-badges">
            <span v-if="player.inJail" class="badge badge-jail" :title="isZh ? '在监狱中' : 'In Jail'">🔒</span>
            <span v-if="player.jailFreeCards > 0" class="badge badge-free" :title="isZh ? '出狱免费卡' : 'Get Out of Jail Free'">🃏×{{ player.jailFreeCards }}</span>
            <span v-if="store.bankruptPlayers.includes(player.id)" class="badge badge-bust">{{ isZh ? '破产' : 'BUST' }}</span>
          </div>
        </div>
    ...
    <div class="action-panel" v-if="store.phase === 'playing' && store.currentPlayer">
      <div class="action-header">
        <span class="font-display text-gold font-semibold text-sm">
          {{ isZh ? `${store.currentPlayer.name} 的回合` : `${store.currentPlayer.name}'s Turn` }}
        </span>
        <span v-if="store.doublesCount > 0" class="doubles-badge">{{ isZh ? `双数 ×${store.doublesCount}` : `Doubles ×${store.doublesCount}` }}</span>
      </div>

      <!-- Jail options -->
      <div v-if="store.currentPlayer.inJail" class="jail-options">
        <p class="jail-info text-orange-300 text-xs mb-2 font-body">
          {{ isZh ? `在监狱中（第 ${store.currentPlayer.jailTurns} 回合）` : `In jail (turn ${store.currentPlayer.jailTurns})` }}
        </p>
        <div class="flex gap-2 flex-wrap">
          ...
          >{{ isZh ? '使用免费卡' : 'Use Free Card' }}</button>
          <button
          ...
          >{{ isZh ? '缴 $50 出狱' : 'Pay $50 Bail' }}</button>
          <button
          ...
          >{{ isZh ? '掷骰（赌双数）' : 'Roll for Doubles' }}</button>
        </div>
      </div>
      ...
      <div v-if="myProps(store.currentPlayer.id).length" class="manage-section">
        <div class="manage-header" @click="showManage = !showManage">
          <span class="text-xs text-ivory/60 font-display">{{ isZh ? '管理地产' : 'Manage Properties' }}</span>
      ...
      <!-- Bankrupt button -->
      <button @click="confirmBankrupt" class="action-btn btn-bankrupt mt-2 w-full text-xs">
        {{ isZh ? '宣告破产' : 'Declare Bankruptcy' }}
      </button>

      <!-- New game button -->
      <button @click="confirmNewGame" class="action-btn btn-newgame mt-1 w-full text-xs">
        🔄 {{ isZh ? '新开一盘' : 'New Game' }}
      </button>
    </div>
  </div>
...
function myProps(playerId) { return store.getPlayerProperties(playerId) }
...
function confirmBankrupt() {
  const msg = isZh.value
    ? `确定要让 ${store.currentPlayer.name} 宣告破产吗？`
...
function confirmNewGame() {
  const msg = isZh.value
    ? '确定放弃当前游戏并新开一盘？\n本局进度将被清除且无法恢复。'
    : 'Abandon current game and start a new one? Progress will be lost.'
  if (confirm(msg)) store.resetGame()
}
</script>

<style scoped>
.player-panel { display: flex; flex-direction: column; gap: 10px; height: 100%; }
.players-grid { display: flex; flex-direction: column; gap: 6px; }
.player-card { background: rgba(0,0,0,0.25); border: 1px solid rgba(201,168,76,0.15); border-radius: 8px; padding: 8px 10px; transition: all 0.2s; }
.player-card.clickable { cursor: pointer; }
.player-card.clickable:hover:not(.active-player) { border-color: rgba(201,168,76,0.35); }
.active-player { border-color: var(--player-color) !important; background: rgba(0,0,0,0.4) !important; box-shadow: 0 0 0 1px var(--player-color), 0 2px 12px rgba(0,0,0,0.4); }
.bankrupt { opacity: 0.4; }
.player-top { display: flex; align-items: center; gap: 8px; }
.player-token-big { font-size: 20px; width: 28px; text-align: center; flex-shrink: 0; }
.player-info { flex: 1; min-width: 0; }
.player-name { font-family: 'Playfair Display', serif; font-size: 13px; font-weight: 600; color: #e2c06a; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
.player-money { font-family: 'Courier Prime', monospace; font-size: 14px; font-weight: 700; color: #f5ead5; }
.player-badges { display: flex; gap: 3px; flex-wrap: wrap; }
.badge { font-size: 10px; padding: 1px 4px; border-radius: 4px; font-family: 'Crimson Text', serif; }
...
.player-properties { display: flex; flex-wrap: wrap; gap: 2px; margin-top: 6px; padding-top: 6px; border-top: 1px solid rgba(255,255,255,0.05); }
.prop-chip { width: 14px; height: 14px; border-radius: 2px; display: flex; align-items: center; justify-content: center; font-size: 8px; border: 1px solid rgba(255,255,255,0.15); }
.prop-mortgaged { opacity: 0.4; filter: grayscale(1); }
.action-panel { background: rgba(0,0,0,0.3); border: 1px solid rgba(201,168,76,0.2); border-radius: 8px; padding: 10px; }
.action-header { display: flex; justify-content: space-between; align-items: center; margin-bottom: 8px; }
.doubles-badge { font-size: 10px; padding: 1px 6px; background: rgba(201,168,76,0.2); color: #e2c06a; border-radius: 4px; font-family: 'Crimson Text', serif; }
.jail-options { margin-bottom: 8px; }
.normal-actions { margin-bottom: 8px; }
.action-btn { padding: 8px 12px; border-radius: 6px; font-family: 'Playfair Display', serif; font-size: 13px; font-weight: 600; transition: all 0.15s; cursor: pointer; }
.btn-roll { background: linear-gradient(135deg, #e2c06a, #c9a84c, #8a6820); color: #2a1a0a; box-shadow: 0 2px 8px rgba(201,168,76,0.3); }
.btn-roll:hover:not(:disabled) { transform: translateY(-1px); box-shadow: 0 4px 12px rgba(201,168,76,0.4); }
.btn-end { background: rgba(245,234,213,0.08); color: #f5ead5; border: 1px solid rgba(245,234,213,0.2); }
...
.manage-header { display: flex; justify-content: space-between; align-items: center; cursor: pointer; padding: 2px 0; }
.manage-list { margin-top: 6px; display: flex; flex-direction: column; gap: 4px; }
.manage-row { display: flex; align-items: center; justify-content: space-between; gap: 4px; }
...
.mini-btn { font-size: 10px; padding: 1px 5px; border-radius: 3px; font-family: 'Courier Prime', monospace; cursor: pointer; transition: all 0.15s; }
